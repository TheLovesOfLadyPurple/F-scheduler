...
def get_model_fn(model, train=False):
    ...
    def model_fn(x, labels, encoder_hidden_states=None):
        ...
        if not train:
            model.eval()
            sample = model(x, labels, encoder_hidden_states=encoder_hidden_states).sample
            return sample
        else:
            model.train()
            return model(x, labels, encoder_hidden_states=encoder_hidden_states).sample
...
def get_noise_fn(sde, model, train=False, continuous=False):
    ...
    if isinstance(sde, sde_lib.VPSDE) and continuous:
        ...
        def noise_fn(x, t, encoder_hidden_states):
